...
#### Social Characteristics ####
  ...
  single_mothers_map  
   ...
   ##education 
    ...
    length(education_vars_18) == length(education_percent_vars_18) 
    length(education_vars_18) == length(education_vars_names_18)    
  ...
  #### Economic Characteristics ####
    ...
    income_spread_map <- median_household_income_map + mapview(mean_household_income_map, legend = FALSE)
...
#### Housing Characteristics ####
 ...
 # transportation per housing unit
    ...
    df_transp_housing_tract_counts_18 <- combined_tract_dp_18 %>%
      filter(variable %in% transp_housing_vars_18)
    
    df_transp_housing_county_counts_18 <- combined_county_dp_18 %>%
      filter(variable %in% transp_housing_vars_18)
    
    df_transp_housing_tract_percent_18 <- combined_tract_dp_18 %>%
      filter(variable %in% transp_housing_percent_vars_18)
    
    df_transp_housing_county_percent_18 <- combined_county_dp_18 %>%
      filter(variable %in% transp_housing_percent_vars_18)
    
    df_transp_housing_kpt_counts_18 <- kpt_data_profile_18 %>%
      filter(variable %in% transp_housing_vars_18)
    
    df_transp_housing_kpt_percent_18 <- kpt_data_profile_18 %>%
      filter(variable %in% transp_housing_percent_vars_18)
    ...
    df_transp_housing_kpt_18 <- cbind(transp_housing_vars_names_18, df_transp_housing_kpt_counts_18, df_transp_housing_kpt_percent_18)
    
    transp_housingu_names <- names(df_transp_housing_county_18)
    
    transp_housingu_names_2 <- transp_housingu_names[-c(16,17)]
    
    names(df_transp_housing_kpt_18) <- transp_housingu_names_2
    
    transp_housing_kpt_18_for_map <- left_join(kpt_18, df_transp_housing_kpt_18, by = "GEOID") #this has the location data
    ...
    trimmed_df_transp_housing_kpt_18 <- df_transp_housing_kpt_18[,c(1,5,6,12,13)] # no location data
    ...
    data_table_transp_housing_kpt_18 <- trimmed_df_transp_housing_kpt_18
    ...
    transp_housing_table_kpt_names <- c("Housing Unit Measure", "Count", "Margin of Error", "Percent", "Percent Margin of Error")
    ...
    names(data_table_transp_housing_kpt_18) <- transp_housing_table_kpt_names
    
    ##make the data tables
    
    transp_housing_tract_data_table_18 <- datatable(data_table_transp_housing_tract_18, caption = "Transportation for Housing Unit Measures of Census Tract Groups for Sullivan and Hawkins County, Tennessee 2018")
    
    transp_housing_county_data_table_18 <- datatable(data_table_transp_housing_county_18, caption = "Transportation for Housing Unit Measures for Sullivan and Hawkins County, Tennessee 2018")
    
    transp_housing_kpt_data_table_18 <- datatable(data_table_transp_housing_kpt_18, caption = "Transportation for Housing Unit Measures for Kingsport, Tennessee 2018")
    
    transp_housing_tract_data_table_18
    
    transp_housing_county_data_table_18
    
    transp_housing_kpt_data_table_18
    
    
    ##service transp_housingupations map 
    ...
    trimmed_transp_housing_kpt_18_for_map <- transp_housing_kpt_18_for_map[,c(15,16,18,19,25,26)]
    
    transp_housing_kpt_map_names <- c("Housing Unit Measure", "Kingsport", "Count", "Count MOE", "Percent", "Percent MOE", "geometry")
    ...
    names(trimmed_transp_housing_kpt_18_for_map) <- transp_housing_kpt_map_names
    ...
    zero_vehicles_kpt <- trimmed_transp_housing_kpt_18_for_map %>%
      filter(`Housing Unit Measure` == "Occupied Housing Units with Zero Vehicles Available")
    
    zero_vehicles_map <- mapview(list(zero_vehicles_county, zero_vehicles_tract, zero_vehicles_kpt),
                                 zcol = list("Percent", "Percent", NULL),
    ...
    df_heating_tract_counts_18 <- combined_tract_dp_18 %>%
      filter(variable %in% heating_vars_18)
    
    df_heating_county_counts_18 <- combined_county_dp_18 %>%
      filter(variable %in% heating_vars_18)
    
    df_heating_tract_percent_18 <- combined_tract_dp_18 %>%
      filter(variable %in% heating_percent_vars_18)
    
    df_heating_county_percent_18 <- combined_county_dp_18 %>%
      filter(variable %in% heating_percent_vars_18)
    
    df_heating_kpt_counts_18 <- kpt_data_profile_18 %>%
      filter(variable %in% heating_vars_18)
    
    df_heating_kpt_percent_18 <- kpt_data_profile_18 %>%
      filter(variable %in% heating_percent_vars_18)
    ...
    df_heating_kpt_18 <- cbind(heating_vars_names_18, df_heating_kpt_counts_18, df_heating_kpt_percent_18)
    
    heatingu_names <- names(df_heating_county_18)
    
    heatingu_names_2 <- heatingu_names[-c(16,17)]
    
    names(df_heating_kpt_18) <- heatingu_names_2
    
    heating_kpt_18_for_map <- left_join(kpt_18, df_heating_kpt_18, by = "GEOID") #this has the location data
    ...
    trimmed_df_heating_kpt_18 <- df_heating_kpt_18[,c(1,5,6,12,13)] # no location data
    ...
    data_table_heating_kpt_18 <- trimmed_df_heating_kpt_18
    ...
    heating_table_kpt_names <- c("Housing Unit Measure", "Count", "Margin of Error", "Percent", "Percent Margin of Error")
    
    names(data_table_heating_tract_18) <- heating_table_tract_names
    
    names(data_table_heating_county_18) <- heating_table_county_names
    
    names(data_table_heating_kpt_18) <- heating_table_kpt_names
    
    ##make the data tables
    
    heating_tract_data_table_18 <- datatable(data_table_heating_tract_18, caption = "Heating and Misc Housing Unit Measures of Census Tract Groups for Sullivan and Hawkins County, Tennessee 2018")
    
    heating_county_data_table_18 <- datatable(data_table_heating_county_18, caption = "Heating and Misc Housing Unit Measures for Sullivan and Hawkins County, Tennessee 2018")
    
    heating_kpt_data_table_18 <- datatable(data_table_heating_kpt_18, caption = "Heating and Misc Housing Unit Measures for Kingsport, Tennessee 2018")
    
    heating_tract_data_table_18
    
    heating_county_data_table_18
    
    heating_kpt_data_table_18
    ...
    trimmed_heating_kpt_18_for_map <- heating_kpt_18_for_map[,c(15,16,18,19,25,26)]
    
    heating_kpt_map_names <- c("Heating and Misc Housing Unit Measure", "Kingsport", "Count", "Count MOE", "Percent", "Percent MOE", "geometry")
    
    names(trimmed_df_heating_county_18) <- heating_county_map_names
    
    names(trimmed_df_heating_tract_18) <- heating_tract_map_names
    
    names(trimmed_heating_kpt_18_for_map) <- heating_kpt_map_names
    ...
    no_heat_source_kpt <- trimmed_heating_kpt_18_for_map %>%
      filter(`Heating and Misc Housing Unit Measure` == "Occupied Housing Units No Heat Source")
    
    no_heat_source_map <- mapview(list(no_heat_source_county, no_heat_source_tract, no_heat_source_kpt),
                                  zcol = list("Percent", "Percent", NULL),
                                  layer.name = list("Percent of Occupied Housing Units with No Heat Source County", "Percent of Occupied Housing Units with No Heat Source Tract", "Percent of Occupied Housing Units with No Heat Source City"),
                                  legend = list(FALSE, TRUE, FALSE))
