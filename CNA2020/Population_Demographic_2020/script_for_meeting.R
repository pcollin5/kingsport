...
Carter_age_change <- Carter_age_change[,-c(4,5)] #still has location
...
Location <- c("US", "TN", "Carter", "Greene", "Johnson", "Hancock", "Hawkins", "Sullivan", "Washington", "Unicoi")

small_named_final_pop_table <- cbind(Location, final_pop_table)

pop_for_plot <- small_named_final_pop_table[,-2]

perc_pop_plot <-pop_for_plot %>%
  ggplot(aes(x = reorder(Location,-`ACS 2018 Estimate`), y = `Pop. Percent Change`)) +
  geom_col(fill = brewer.pal(10, "Paired"))+
  coord_cartesian(ylim = c(-6,6))+
  xlab("Location")+
  ggtitle("Percent Population Change", subtitle = "2010 Census to 2018 ACS")



###age group###

pretty_named_age_groups <- cbind(Location, age_groups_for_table)

pretty_age_groups <- pretty_named_age_groups[,-2]  

hawkins_age_group_plot <- pretty_age_groups %>% 
  filter(Location == "Hawkins") %>%
  gather("id", "value",11:13) %>%
  ggplot(.,aes(x = id,y = value))+
  geom_col(fill = brewer.pal(3, "Dark2"))+
  coord_cartesian(ylim = c(-30,30))+
  ylab("Percent Change") +
  xlab("Age Metric")+
  ggtitle("Hawkins County Age Group Percent Changes", subtitle = "2010 Census to 2018 ACS")

hawkins_age_group_plot

## hispanic changes

pretty_sex_groups_for_table <- cbind(Location, sex_groups_for_table)

pretty_sex_groups <- pretty_sex_groups_for_table[,-2]

v2010 <- pretty_sex_groups[,c(1,2,3,4)]
v2018 <- pretty_sex_groups[,c(1,5,6,7)]

pretty_sex_groups %>%
  filter(Location != "US") %>%
  filter(Location != "TN") %>%
  ggplot(aes(x = Location, y = `2010 Census: Total` ,fill = (`2010 Census: Male`/ `2010 Census: Total`)))+
  geom_bar(stat = "identity")

### kable tables, figure out how to add in a png to the latex ###

kable_pop_table <- kable(final_pop_table, booktabs = TRUE, caption = "Population Changes") %>%
  kable_styling(latex_options = c("striped", "scale_down"))

kable_pop_table

kable_age_group_table <- kable(age_groups_for_table, caption = "Age Group Changes") %>%
  kable_styling(latex_options = c("scale_down", "repeat_header"))

kablea_race_table <- race_groups_for_table %>%
  mutate("Percent Change: White" = cell_spec(`Percent Change: White`, "latex", color = ifelse(`Percent Change: White` < 0, "Red", "Green"))) %>%
  kable("latex", booktabs = TRUE)%>%
  kable_styling(latex_options = c("striped", "scale_down"))

kablea_race_table

kable_gender_table <- kable(sex_groups_for_table, booktabs = TRUE, caption = "Sex Demographic Changes") %>%
  kable_styling(latex_options ="scale_down")

kable_gender_table

kable_hispanic_table <- kable(hisp_groups_for_table, booktabs = TRUE, caption = "Hispanic Group Changes") %>%
  kable_styling(latex_options = c("scale_down"), font_size = 5)

kable_hispanic_table

kable_housing_table <- kable(hous_groups_for_table, booktabs = TRUE, caption = "Housing Unit Changes") %>%
