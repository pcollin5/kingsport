---
title: "Some Demographic Changes from 2010"
author: "Paul Collins"
date: "06/03/2020"
output: pdf_document
---
...
This shows how plots look in the PDF, ignore the scaling as I ran out of time doing this, but its an easy fix.
...
## Hawkins County Age Group Changes Plot

Population is getting a lot older

```{r hawkins county age group plot, echo = FALSE}
...
## Add extra Images from Other Places in 

```{r age group by county, echo = FALSE, fig.cap = "Age Group by County", out.width = '100%'}

include_graphics("age_group_plots.png")
...
```{r static map, echo = FALSE, fig.cap = "Median Age by Census Tract", out.width = '100%'}

include_graphics("median_age_mape.png")
...
kable_pop_table1 <- kable(final_pop_table, booktabs = TRUE, caption = "Population Changes") %>%
...
kable_age_group_table

```

### Racial Group Table

```{r racial groups, echo = FALSE}
...
race_group_total_changes_only <- race_groups_for_table[,c(1,14:19)]

race_group_percent_changes_only <- race_groups_for_table[,c(1,20:25)]
...
names(race_group_total_changes_only) <- total_short_names

names(race_group_percent_changes_only) <- percent_short_names

kable_race_changes_total_table <- race_group_total_changes_only %>%
  mutate("Total Change: White" = cell_spec(`Total Change: White`, color = ifelse(`Total Change: White` < 0, "red", "green"))) %>%
  mutate("Total Change: AA" = cell_spec(`Total Change: AA`, color = ifelse(`Total Change: AA` < 0, "red", "green"))) %>%
  mutate("Total Change: AIAN" = cell_spec(`Total Change: AIAN`, color = ifelse(`Total Change: AIAN` < 0, "red", "green"))) %>%
  mutate("Total Change: Asian" = cell_spec(`Total Change: Asian`, color = ifelse(`Total Change: Asian` < 0, "red", "green"))) %>%
  mutate("Total Change: NHPI" = cell_spec(`Total Change: NHPI`, color = ifelse(`Total Change: NHPI` < 0, "red", "green"))) %>%
...
kable_race_changes_total_table

kable_race_changes_percent_table <- race_group_percent_changes_only %>%
  mutate("Percent Change: White" = cell_spec(`Percent Change: White`, color = ifelse(`Percent Change: White` < 0, "red", "green"))) %>%
  mutate("Percent Change: AA" = cell_spec(`Percent Change: AA`, color = ifelse(`Percent Change: AA` < 0, "red", "green"))) %>%
  mutate("Percent Change: AIAN" = cell_spec(`Percent Change: AIAN`, color = ifelse(`Percent Change: AIAN` < 0, "red", "green"))) %>%
  mutate("Percent Change: Asian" = cell_spec(`Percent Change: Asian`, color = ifelse(`Percent Change: Asian` < 0, "red", "green"))) %>%
  mutate("Percent Change: NHPI" = cell_spec(`Percent Change: NHPI`, color = ifelse(`Percent Change: NHPI` < 0, "red", "green"))) %>%
...
kable_race_changes_percent_table
...
kable_gender_table <- kable(sex_groups_for_table, booktabs = TRUE, caption = "Sex Demographic Changes") %>%
  kable_styling(latex_options =c("scale_down", "HOLD_positon"))
...
age_sex_kable <- kable(age_sex_df2, caption = "Sex and Age 2018 ACS") %>%
    kable_styling(latex_options = c("scale_down", "HOLD_position"))
