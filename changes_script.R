...
Greene_hisp_change$hisp_names <- c("Total", "Non-Hispanic", "Hispanic", "Hispanic", "Total", "Non-Hispanic")
...
Greene_hisp_18_value <- Greene_hisp_18_value[c(2,3,1),]
...
Male <- (sex_groups_for_table$`2018 ACS: Male`/ sex_groups_for_table$`2018 ACS: Total`)*100

Female <- (sex_groups_for_table$`2018 ACS: Female`/sex_groups_for_table$`2018 ACS: Total`)*100
...
###these are going to be plots for racial groups/changes

hisp_groups_for_plot <- hisp_groups_for_table

hisp_groups_for_plot$Location <- c("US", "TN", "Carter", "Greene", "Johnson", "Hancock", "Hawkins", "Sullivan", "Washington", "Unicoi")

hisp_percent_plot <- hisp_groups_for_plot[,c(1, 10, 11, 12)]

long_hisp_for_plot <- hisp_percent_plot %>%
  pivot_longer(-Location, names_to = "Ethnic Group", values_to = "Percent Change" )

all_counties_hispanic_plots <- long_hisp_for_plot %>%
  ggplot(aes(x = Location, y = `Percent Change`, fill = `Ethnic Group`))+
  geom_bar(stat = "identity", position = "dodge")

long_hisp_for_plot$Location <- factor(long_hisp_for_plot$Location, levels = c("US", "TN", "Carter", "Greene", "Hawkins", "Johnson",
                                                                              "Sullivan", "Unicoi", "Washington"))

minus_hancock_hispanic_plot <- long_hisp_for_plot %>%
  filter(Location != "Hancock") %>%
  ggplot(aes(x = Location, y = `Percent Change`, fill = `Ethnic Group`))+
  geom_bar(stat = "identity", position = "dodge")

minus_hancock_hispanic_plot

#get hancock alone

hancock_hisp <- hisp_groups_for_plot %>%
  filter(Location == "Hancock")

hancock_hisp_total <- hancock_hisp[,c(1,8,9)]

hancock_hisp_perc <- hancock_hisp[,c(1,10,11,12)]

long_hancock_hisp_total <- hancock_hisp_total %>%
  pivot_longer(-Location, names_to ="Ethnic Group", values_to = "Total Change")

long_hancock_hisp_perc <- hancock_hisp_perc %>%
  pivot_longer(-Location, names_to ="Ethnic Group", values_to = "Percent Change")

hancock_hisp_percent_plot <- long_hancock_hisp_perc %>%
  ggplot(aes(x = `Ethnic Group`, y = `Percent Change`, fill = `Ethnic Group`))+
  geom_bar(stat = "identity")

hancock_hisp_count_plot <- long_hancock_hisp_total %>%
  ggplot(aes(x = `Ethnic Group`, y = `Total Change`, fill = `Ethnic Group`))+
  geom_bar(stat = "identity")+
  ylim(-350,350)

#racial percent change bar chart

names(race_groups_for_table)

percent_race_change <- race_groups_for_table[,c(1, 20:25)]

percent_race_change$Location <- c("US", "TN", "Carter", "Greene", "Johnson", "Hancock", "Hawkins", "Sullivan", "Washington", "Unicoi")

total_race_changes <- race_groups_for_table[,c(1,14:19)]

total_race_changes$Location <- c("US", "TN", "Carter", "Greene", "Johnson", "Hancock", "Hawkins", "Sullivan", "Washington", "Unicoi")

long_percent_race_change <- percent_race_change %>%
  pivot_longer(-Location, names_to = "Racial Group", values_to = "Percent Change")

long_total_race_change <- total_race_changes %>%
  pivot_longer(-Location, names_to = "Racial Group", values_to = "Total Change")

long_percent_race_change$Location <- factor(long_percent_race_change$Location, levels = c("US", "TN", "Carter", "Greene", "Johnson", "Hancock", "Hawkins", "Sullivan", "Washington", "Unicoi"))

all_counties_racial_change_bar <- long_percent_race_change %>%
  ggplot(aes(x = Location, y = `Percent Change`, fill = `Racial Group`)) + 
  geom_bar(stat = "identity", position = "dodge")


minus_carter_unicoi_racial_change_bar <- long_percent_race_change %>%
  filter(!(Location == "Carter" | Location == "Unicoi")) %>%
  ggplot(aes(x = Location, y = `Percent Change`, fill = `Racial Group`)) + 
  geom_bar(stat = "identity", position = "dodge")

carter_unicoi_racial_change_bar <- long_percent_race_change %>%
  filter((Location == "Carter" | Location == "Unicoi")) %>%
  ggplot(aes(x = Location, y = `Percent Change`, fill = `Racial Group`)) + 
  geom_bar(stat = "identity", position = "dodge")

carter_unicoi_total_change_bar <- long_total_race_change %>%
  filter((Location == "Carter" | Location == "Unicoi")) %>%
  ggplot(aes(x = Location, y = `Total Change`, fill = `Racial Group`)) + 
  geom_bar(stat = "identity", position = "dodge")+
  ylim(-1500,1500)

carter_unicoi_total_change_bar

### race groups change table

names(race_groups_for_table)

race_group_changes_only <- race_groups_for_table[,c(1,14:25)]

names(race_group_changes_only)


kable_race_changes_table <- race_group_changes_only %>%
...
percent_short_names <- c("Location", "Percent Change: White", "Percent Change: AA", "Percent Change: AIAN", "Percent Change: Asian",
                         "Percent Change: NHPI", "Percent Change: Two or More Races")

###age and sex table

Male <- as.numeric(round((sex_groups_for_table$`2018 ACS: Male`/ sex_groups_for_table$`2018 ACS: Total`)*100, 2)
)
Female <- as.numeric(round((sex_groups_for_table$`2018 ACS: Female`/sex_groups_for_table$`2018 ACS: Total`)*100, 2)
)
Over_18 <- as.numeric(round((age_groups_2018$`2018 ACS: Age over 18 Years`/age_groups_2018$`2018 ACS: All Ages`)*100, 2)
)
Under_5 <- as.numeric(round((age_groups_2018$`2018 ACS: Age 0-5 Years`/age_groups_2018$`2018 ACS: All Ages`)*100, 2)
)
Median_Age <- as.numeric(age_groups_2018$`2018 ACS: Median Age`)

Location_Names <- c("US", "TN", "Carter", "Greene", "Johnson", "Hancock",
                    "Hawkins", "Sullivan", "Washington", "Unicoi")

age_sex_table <- cbind(Male, Female, Over_18, Under_5, Median_Age)

age_sex_df <-as.data.frame(age_sex_table)
...
gender_percent <- age_sex_df2[,c(1,2,3)]



gender_long <- gender_percent %>%
  pivot_longer(-Location, names_to = "Gender", values_to = "Percent" )



gender_long$Geography <- c("US", "US", "TN", "TN", "Carter", "Carter", "Greene", "Greene",
                           "Johnson", "Johnson", "Hancock", "Hancock", "Hawkins", "Hawkins", "Sullivan",
                           "Sullivan", "Washington", "Washington", "Unicoi", "Unicoi")
gender_long$Location <- factor(gender_long$Location, levels = c("US", "TN", "Carter", "Greene", "Johnson",
                                                                  "Hancock", "Hawkins", "Sullivan", "Washington", "Unicoi"))

gender_percent_plot <- gender_long %>%
ggplot(aes(x = Location, y = Percent, fill = Gender))+
  geom_bar(stat = "identity", position = "dodge")+
  scale_y_continuous(limits = c(0,100))



###need percent of each age group###



age_groups_2018$`2018 ACS: % < 5` <- round((age_groups_2018$`2018 ACS: Age 0-5 Years`/age_groups_2018$`2018 ACS: All Ages`)*100,2)

age_groups_2018$`2018 ACS: % 18+` <- round((age_groups_2018$`2018 ACS: Age over 18 Years`/age_groups_2018$`2018 ACS: All Ages`)*100,2)

age_groups_2018$`2018 ACS: % 65+` <- round((age_groups_2018$`2018 ACS: Age over 65 Years`/age_groups_2018$`2018 ACS: All Ages`)*100,2)

new_age_changes_table <- cbind(age_groups_2018[,c(1,8,9,10,7)], age_group_changes_2010_2018[,c(8,11,10,12)])

row.names(new_age_changes_table) <- c()

new_age_changes_table$Location_Names <- Location_Names

names(new_age_changes_table)

new_age_changes_table2 <- new_age_changes_table[,c(10, 2:9)]

new_table_names <- c("Location", "2018 ACS: < 5", "2018 ACS: 18+", "2018 ACS: 65+", "2018 ACS: Med. Age",
                     "% Change: < 5", "% Change: 18+", "% Change: 65+", "% Change: Med. Age")

names(new_age_changes_table2) <- new_table_names

long_percent_changes <- new_age_changes_table2 %>%
  select(`Location`, `% Change: < 5`, `% Change: 18+`, `% Change: 65+`, `% Change: Med. Age`) %>%
  pivot_longer(-Location, names_to = c("Age Group"), values_to = c("Percent Change"))

long_percent_changes$Location <- factor(long_percent_changes$Location, levels = c("US", "TN", "Carter", "Greene", "Johnson", "Hancock",
                                                                                  "Hawkins", "Sullivan", "Washington", "Unicoi"))
