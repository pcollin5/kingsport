...
####tract_dp_2018, County_dp_2018, DP03_0131####

total_u5_tract <- tract_dp_2018 %>%
  filter(variable == "DP05_0005")

total_u5_county <- County_dp_2018 %>%
  filter(variable == "DP05_0005")
...
county_u5_poverty_18 <-County_dp_2018 %>%
  filter(variable == "DP03_0131P")
      ...
      ####this is messing up, st_join is fucked for some reason###
u5_tract <- st_join(total_u5_tract, tract_u5_poverty_18, by = "NAME")

u5_county <- st_join(total_u5_county, county_u5_poverty_18, by = "NAME")

dim(u5_county)

dim(total_u5_county)
dim(county_u5_poverty_18)

head(u5_tract)

names(u5_tract)

trimmed_u5_tract <- u5_tract[,c(2,4,5,11,12)]

trimmed_u5_county <- u5_county[,c(2,4,5,11,12)]

names(trimmed_u5_tract)
head(trimmed_u5_tract)

u5_names <- c("Location", "Count", "Count Margin of Error", "Percent in Poverty", "Percent Margin of Error", "geometry")

names(trimmed_u5_tract) <- u5_names

names(trimmed_u5_county) <- u5_names

map_u5_tract <- mapview(trimmed_u5_tract, zcol = "Percent in Poverty", layer.name = "Under 5 Years of Age Percent in Poverty")

map_u5_county <- mapview(trimmed_u5_county, zcol = "Percent in Poverty", layer.name = "County Wide Under 5 Years of Age Percent in Poverty", legend = FALSE)

full_map <- map_u5_tract + map_u5_county

full_map

dt_trimmed_u5_tract <- st_set_geometry(trimmed_u5_tract, NULL)

dt_trimmed_u5_county <- st_set_geometry(trimmed_u5_county, NULL)

full_tract_datatable <- datatable(dt_trimmed_u5_tract, caption = "Children under 5 Totals and Percent in Poverty at Census Tract Level")

full_county_datatable <- datatable(dt_trimmed_u5_county, caption = "Children under 5 Totals and Percent in Poverty at County Level")

full_tract_datatable

full_county_datatable

dim(trimmed_u5_county)



####this is great, but I want total kids####
...
u5_tract_map <- mapview(trimmed_tract_u5_poverty_18, zcol = "Percent", layer.name = "Under 5 Years of Age Percent in Poverty")
...
location_map <- mapview(trimmed_tract_u5_poverty_18, zcol = "Location", layer.name = "Tract Number", legend = FALSE)
